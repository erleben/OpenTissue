...
{
  {
    ...
    // We either have to modify the read function to extract the chunks or find an image that does not contain any of the
    // chunks so that we can do fair a comparision. For now this tests that there is no mayor exceptions comming from libpng
    // while reading/writting a png image.
