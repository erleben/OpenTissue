    ...
    case 'i':
      {
        ...
        real_type density = 1000;
        {
          ...
          mesh_type::node_iterator node = m_mesh1.node_begin();
          for ( ; node != m_mesh1.node_end(); ++node)
          {
...

        {
          ...
          OpenTissue::fem::update_original_coord(m_mesh2.node_begin(),m_mesh2.node_end());
          mesh_type::node_iterator node = m_mesh2.node_begin();
          for ( ; node != m_mesh2.node_end(); ++node)
          {
            if(node->m_model_coord(0) < 0.01)
              node->m_fixed = true;
          }
  ...
  {
    {
      ...
      node = m_mesh2.node_begin();
      for ( ; node != m_mesh2.node_end(); ++node)
        node->m_f_external = vector3_type(0.0, -(node->m_mass * m_gravity), 0.0);
    }

    OpenTissue::fem::simulate(m_mesh1,0.01,m_stiffness_warp_on); 
