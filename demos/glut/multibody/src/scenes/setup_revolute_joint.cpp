...
//
#include "setup_revolute_joint.h"

void setup_revolute_joint(Data & data)
{
  ...
  data.m_bodies[2].set_spin(vector3_type(0,0,4));
  ...
  data.m_hinge.connect(data.m_socket_A,data.m_socket_B);

  data.m_hinge.set_frames_per_second(value_traits::one()/timestep);
  data.m_hinge.set_error_reduction_parameter(0.8);
  ...
  data.m_hinge.set_limit(data.m_angular_limit1);
  ...
  data.m_hinge.set_motor(data.m_angular_motor1);

  data.m_configuration.add(&data.m_hinge);
