...
void setup_domino_circle(Data & data)
{
  ...
  real_type radius = 10*W;
  // TODO: Needs a proper cast to size_type
  size_type N = static_cast<size_type>( (value_traits::two()*value_traits::pi()*radius)/(value_traits::two()*H) );
  real_type theta = 0;
  real_type delta_theta = value_traits::two()*value_traits::pi()/N;
