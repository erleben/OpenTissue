...
//
#include "setup_wheel_joint.h"

void setup_wheel_joint(Data & data)
{
  ...
  data.m_bodies[2].set_position(vector3_type(value_traits::zero(),7,0-value_traits::two()));
  Q.Ry(value_traits::pi()/value_traits::two());
  data.m_bodies[2].set_orientation(Q);
  data.m_bodies[2].set_velocity(vector3_type(value_traits::zero(),value_traits::zero(),value_traits::zero()));
  ...
  data.m_wheel.connect(data.m_socket_A,data.m_socket_B);
  data.m_wheel.set_frames_per_second(value_traits::one()/timestep);
  data.m_wheel.set_error_reduction_parameter(0.8);
  ...
  data.m_wheel.set_steering_limit(data.m_angular_limit1);
  ...
  data.m_wheel.set_steering_motor(data.m_angular_motor1);
  ...
  data.m_angular_motor2.set_desired_speed(10.0);
  data.m_wheel.set_wheel_motor(data.m_angular_motor2);

  data.m_configuration.add(&data.m_wheel);
