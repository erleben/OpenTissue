...
void setup_jamm2(Data & data)
{
  ...
  data.m_small_box.set(vector3_type(value_traits::zero(),value_traits::zero(),value_traits::zero()),R,vector3_type(.5,1.5,.5));
  ...
  data.m_box.set(vector3_type(value_traits::zero(),value_traits::zero(),value_traits::zero()),R,vector3_type(2.0,.5,0.5));
  OpenTissue::geometry::compute_box_mass_properties(data.m_small_box.ext(),density,mass,diag_inertia);
  I= OpenTissue::math::diag(diag_inertia(0),diag_inertia(1),diag_inertia(2));

  data.m_bodies[i].attach(&data.m_gravity);
  data.m_bodies[i].set_position(vector3_type(0,-2.0,0));
  data.m_bodies[i].set_mass(mass);
  data.m_bodies[i].set_inertia_bf(I);
  ...
  data.m_bodies[i].attach(&data.m_gravity);
  data.m_bodies[i].set_position(vector3_type(-0.5,-1.0,0));
  data.m_bodies[i].set_mass(mass);
  data.m_bodies[i].set_inertia_bf(I);
  ...
  data.m_bodies[i].attach(&data.m_gravity);
  data.m_bodies[i].set_position(vector3_type(0.0,0.0,0));
  data.m_bodies[i].set_mass(mass);
  data.m_bodies[i].set_inertia_bf(I);
  data.m_bodies[i].set_geometry(&data.m_sphere[0]);
  data.m_configuration.add(&data.m_bodies[i]);
  ...
  data.m_gravity.set_acceleration(vector3_type(value_traits::zero(),-9.81,value_traits::zero()));
