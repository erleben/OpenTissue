...
protected:
  ...
  typedef  OpenTissue::polymesh::PolyMesh<>               surface_mesh;

  grid_type                           m_phi;
  surface_mesh                       m_surface;
  surface_mesh                       m_isosurface;
...
protected:
  ...
  void doit(unsigned int const & choice)
  {
    ...
    watch.start();
    OpenTissue::t4_gpu_scan(m_surface,thickness, m_phi);
    watch.stop();
    std::cout << "|V| = " 
...
public:
  ...
  char const * do_get_title() const { return "T4 GPU Scan Demo Application"; }
