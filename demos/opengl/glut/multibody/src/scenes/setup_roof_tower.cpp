...
void setup_roof_tower(Data & data)
{
  ...
  data.m_bodies.resize(643);


  matrix3x3_type R;
  R = OpenTissue::math::diag(value_traits::one());
  data.m_box.set(vector3_type(value_traits::zero(),value_traits::zero(),value_traits::zero()),R,vector3_type(10,10,.5));
  ...
  I= OpenTissue::math::diag(diag_inertia(0),diag_inertia(1),diag_inertia(2));
  data.m_bodies[i].set_position(vector3_type(0,0,H*2.));
  data.m_bodies[i].attach(&data.m_gravity);
